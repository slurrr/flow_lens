...
def _boot_screen(stdscr: "curses.window") -> None:
    h, w = stdscr.getmaxyx()
    spinner = ["⠋","⠙","⠹","⠸","⠼","⠴","⠦","⠧","⠇","⠏"]
    i = 0
    ...
    while not state["done"]:
        ...
        # center art
        art_h = len(BOOT_ART)
        art_w = max(len(l) for l in BOOT_ART)
        y0 = max(0, (h // 2) - (art_h // 2) - 2)
        x0 = max(0, (w // 2) - (art_w // 2))

        for dy, line in enumerate(BOOT_ART):
            if 0 <= y0 + dy < h:
                stdscr.addstr(y0 + dy, x0, line[: max(0, w - x0)])

        # progress bar
        bar_w = min(40, w - 10)
        filled = int(bar_w * state["progress"])
        bar = "█" * filled + "░" * (bar_w - filled)

        py = y0 + art_h + 1
        if py < h:
            stdscr.addstr(py, x0, f"[{bar}] {int(state['progress']*100):3d}%")

        # phase + spinner
        py2 = py + 1
        if py2 < h:
            stdscr.addstr(py2, x0, f"{spinner[i % len(spinner)]} {state['phase']}")
    ...
    for dy, line in enumerate(BOOT_ART):
        if dy < h:
            stdscr.addstr(dy, 0, line[:w])
    stdscr.addstr(len(BOOT_ART) + 1, 0, "[████████████████████████████] 100%")
    stdscr.addstr(len(BOOT_ART) + 2, 0, "ready")
    stdscr.refresh()
    time.sleep(0.2)
