...
- Phase 1 model is opposition-primary: no time decay in quiet periods; `S` unwinds when opposing input arrives.
- If dot reacts but `S` barely moves in trends: raise `persist_gain_per_second`.
- If `S` drifts in chop from tiny pulses: raise `persist_input_deadband` (start very small).
- If `S` overshoots or feels too jumpy across variable cadence: lower `persist_gain_per_second`.
- Healthy trend feel: `Y_raw` pulses, `Y_s` holds direction, `S` drifts steadily, `dS/s` mostly same sign as `Y_raw`.
- Healthy chop feel: `Y_raw` flips often, `Y_s` near zero, `S` mostly holds unless opposition accumulates.
